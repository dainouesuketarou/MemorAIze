  ...
  return (
    ...
    >
      ...
      <div className="grid gap-6">
        {loading ? (
          <div className="text-center text-muted-foreground py-10">カードを取得中...</div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {cards.map((card) => (
              <Card 
                ...
                className={cn(
                  ...
                  card.status === 'MASTERED' && "bg-primary/5 border-primary/20",
                  card.status === 'STRUGGLING' && "bg-destructive/5 border-destructive/20"
                )}
              >
            ...
            <button type="button" className="block w-full" onClick={() => setModalOpen(true)}>
      ...
      <Dialog open={modalOpen} onOpenChange={setModalOpen}>
        <DialogContent>
          ...
          <Tabs defaultValue={tab} onValueChange={v => setTab(v as 'ai'|'manual')} className="w-full">
            ...
            <TabsContent value="manual">
              <CardAddManualForm deckId={deckId} onSuccess={handleCardAddSuccess} />
            </TabsContent>
          </Tabs>
          <DialogClose asChild>
