...
export function DeckList({ decks, groupMode, groups, setDecks }: DeckListProps) {
  // タブ（進捗フィルタ）
  const [filter, setFilter] = useState<'all'|'inProgress'|'completed'|'notStarted'>('all');
  // ソート
  const [sort, setSort] = useState<'recent'|'alphabetical'|'cardCount'>('recent');
        ...
        <div className="flex gap-2 items-center">
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button variant="outline" size="sm">並び替え</Button>
            </DropdownMenuTrigger>
          ...
          >
            ...
            <CardContent>
              <div className="flex items-center gap-2 text-sm text-muted-foreground mb-3">
                ...
                {deck.lastStudied && (
                  <>
                    ...
                    <span>最終学習日: {formatRelativeTime(deck.lastStudied)}</span>
                  </>
                )}
              </div>
              {/* 学習進捗バーと割合表示 */}
              {deck.cards ? (() => {
                ...
                const masteredCount = deck.cards.filter((card: { status: string }) => card.status === 'MASTERED').length;
                const strugglingCount = deck.cards.filter((card: { status: string }) => card.status === 'STRUGGLING').length;
                const unlearnedCount = deck.cards.filter((card: { status: string }) => card.status === 'UNLEARNED').length;
                ...
                return (
                  <div className="space-y-2">
                    ...
                    <div className="w-full h-4 bg-gray-200 rounded-full flex overflow-hidden">
                      <div style={{ width: `${masteredPercentage}%` }} className="bg-[#4ade80] h-4" />
                      <div style={{ width: `${strugglingPercentage}%` }} className="bg-[#f87171] h-4" />
                      <div style={{ width: `${unlearnedPercentage}%` }} className="bg-[#9ca3af] h-4" />
                    </div>
                    <div className="flex justify-between text-xs mt-1">
                      <span className="text-[#4ade80]">覚えた {masteredPercentage}%</span>
                      <span className="text-[#f87171]">苦手 {strugglingPercentage}%</span>
                      <span className="text-[#9ca3af]">未学習 {unlearnedPercentage}%</span>
                    </div>
                  </div>
                );
              })() : (
            ...
            <CardFooter className="flex justify-between">
              <Link href={`/deck/${deck.id}`} className="w-full">
                <Button className="w-full">
                  <Play className="mr-2 h-4 w-4"/>学習に進む
                </Button>
              </Link>
            </CardFooter>
          </Card>
        ...
        <DialogContent>
          ...
          {selectedDeck && (
            <div className="flex flex-wrap gap-2">
              {groups.map(group => {
                const checked = selectedDeck.groups.some(g => g.id === group.id);
                return (
                  <label key={group.id} className="cursor-pointer">
                    <input
                      type="checkbox"
                      checked={checked}
                      onChange={()=>toggleDeckGroup(selectedDeck.id, group.id)}
                      className="hidden"
                    />
                    <span
                      className={`inline-block px-4 py-2 rounded select-none text-sm font-medium
                        ${checked ? 'bg-purple-200 text-purple-800' : 'bg-gray-200 text-gray-500'}
